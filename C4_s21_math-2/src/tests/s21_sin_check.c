...
START_TEST(s21_sin_test_zero_neg_zero) {
  double x = 0.0;
  ck_assert_ldouble_eq_tol(s21_sin(x), sin(x), 1e-6);
  double x_1 = S21_NEGZERO;
  ck_assert_ldouble_eq_tol(s21_sin(x_1), sin(x_1), 1e-6);
}
END_TEST
...
START_TEST(s21_sin_test_1) {
  ...
  for (double j = 0; j > -5000; j--) {
    ck_assert_ldouble_eq_tol(s21_sin(j), sin(j), 1e-6);
  }
  double x = S21_PI / 2;
  ck_assert_ldouble_eq_tol(s21_sin(x), sin(x), 1e-6);
  double x_1 = S21_PI / 2 + S21_EPS;
  ck_assert_ldouble_eq_tol(s21_sin(x_1), sin(x_1), 1e-6);
}
END_TEST
...
START_TEST(s21_sin_test_MIN) {
  double x_1 = DBL_MIN;
  ck_assert_ldouble_eq_tol(s21_sin(x_1), sin(x_1), 1e-6);
}
END_TEST

START_TEST(s21_sin_test_INF) {
  double x = S21_INF;
  ck_assert_ldouble_nan(s21_sin(x));
  double x_1 = -S21_INF;
  ck_assert_ldouble_nan(s21_sin(x_1));
}
END_TEST
...
Suite *s21_sin_suite(void) {
  ...
  tcase_add_test(tc_core, s21_sin_test_zero_neg_zero);
  tcase_add_test(tc_core, s21_sin_test_NAN);
  tcase_add_test(tc_core, s21_sin_test_1);
  tcase_add_test(tc_core, s21_sin_test_2);
  tcase_add_test(tc_core, s21_sin_test_MIN);
  tcase_add_test(tc_core, s21_sin_test_INF);
